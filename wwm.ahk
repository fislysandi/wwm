...
NotifHandler(msg="", font="", color="", text_color="", timer="") {
    Run, pythonw.exe "%A_ScriptDir%\Scripts\notifhandler/notifhandler.py" "%msg%" "%font%" "%color%" "%text_color%" "%timer%"
    return
}
...
watch_with_mpv() {
	clipboard_text := clipboard  ; store clipboard contents in a variable
    Run, mpv.exe "%clipboard_text%"  ; launch mpv with clipboard contents as argument
	clipboard := "" ; clear clipboard contents
}



;adjustsysvolume(change="") {
;    scriptPath := A_ScriptDir "\scripts\changesysvolume\changesysvolume.py"
;    Run pythonw.exe "%scriptPath%" "%change%",, Hide
;    current_vol := ""
;	current_vol := Trim(current_vol)
;    NotifHandler("Volume changed to " . current_vol . "%", "Arial", "#FFFFFF", "#000000", "0.5")
;}
...
adjustsysvolume(change="") {
    scriptPath := A_ScriptDir "\scripts\changesysvolume\changesysvolume.py"
    if (StrLen(change) > 0 && SubStr(change, 1, 1) > 0)
        change := "+" . change
    Run pythonw.exe "%scriptPath%" "%change%",, Hide
}
...
; Define the adjustsysvolume() function to adjust the system volume up or down

; Define mouse button hotkeys to adjust the system volume up and down
